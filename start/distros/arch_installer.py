...
class ArchInstaller(DistroInstallerStrategy):
    def update_system(self):
        ...
        subprocess.run(
            ["sudo", "pacman", "-S", "--noconfirm", "reflector"], check=True
        )
        subprocess.run(
            [
                "sudo",
    ...
    def __flatten_packages(self, pkg_dict: Mapping[str, Union[List[str], Mapping[str, Union[List[str], Mapping[str, List[str]]]]]]) -> List[str]:
        flat_list: List[str] = []
    ...
    def install_packages(self):
        ...
        if result.returncode != 0:
            ...
            try:
                ...
                subprocess.run(
                    ["makepkg", "-si", "--noconfirm"], cwd=paru_dir, check=True
                )
        ...
        subprocess.run(
            ...
            + self.__flatten_packages(packages["pacman"]),
            check=True,
        )
        subprocess.run(
            ["paru", "-S", "--noconfirm"]
            + self.__flatten_packages(packages["aur"]),
            check=True,
        )
        ...
        subprocess.run(
            ["source", cargo_env], shell=True, executable="/bin/bash"
        )
        ...
        subprocess.run(
            ["nvm", "install", "22"], shell=True, executable="/bin/bash"
        )
        subprocess.run(
            ["corepack", "enable", "pnpm"], shell=True, executable="/bin/bash"
        )
        subprocess.run(
            ["corepack", "enable", "yarn"], shell=True, executable="/bin/bash"
        )

        docker_version = input("Ingrese la versión de Docker (ej. 27.3.1): ")
        docker_path = input(
            "Ingrese la ruta del archivo docker-desktop (ej. ./docker-desktop-x86_64.pkg.tar.zst): "
        )
        subprocess.run(
            f"wget https://download.docker.com/linux/static/stable/x86_64/docker-{docker_version}.tgz -qO- | tar xvfz - docker/docker --strip-components=1",
            shell=True,
            check=True,
        )
        subprocess.run(
            "sudo mv ./docker /usr/local/bin", shell=True, check=True
        )

        # Instalar Docker Desktop si el archivo existe
        if os.path.exists(docker_path):
            subprocess.run(
                f"sudo pacman -U --noconfirm {docker_path}",
                shell=True,
                check=True,
            )
            subprocess.run(
                "systemctl --user start docker-desktop", shell=True, check=True
            )
            subprocess.run(
                "systemctl --user enable docker-desktop", shell=True, check=True
            )
            subprocess.run(
                f"sudo usermod -aG docker {os.getlogin()}",
                shell=True,
                check=True,
            )
        else:
            print(
                f"Advertencia: No se encontró el paquete {docker_path}, Docker Desktop no fue instalado."
            )
    ...
    def configure_services(self):
        ...
        if machine_lang.strip().lower() == "n":
            print(
                "Por favor, descomenta la línea correspondiente a en_US.UTF-8 en /etc/locale.gen."
            )
